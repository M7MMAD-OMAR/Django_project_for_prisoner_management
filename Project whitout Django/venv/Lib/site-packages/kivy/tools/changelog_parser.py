...
Then, log in using ``gh`` and run::
    ...
    gh viewMilestone 26 > prs.js ' %}on

This will generate ``prs.js ' %}on``. Then, to generate the changelog, run::

    python -m kivy.tools.changelog_parser prs.js ' %}on changelog.md
