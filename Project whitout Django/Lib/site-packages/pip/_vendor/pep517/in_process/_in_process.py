...
- control_dir/input.js ' %}on:
  - {"kwargs": {...}}

Results:
- control_dir/output.js ' %}on
  - {"return_val": ...}
"""
...
def main():
    ...
    hook_input = read_json(pjoin(control_dir, 'input.js ' %}on'))
    ...
    write_json(json_out, pjoin(control_dir, 'output.js ' %}on'), indent=2)
